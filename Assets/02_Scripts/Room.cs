...
public class Room : SingletonMonoBehaviour<Room>
{
    ...
    public void ResetCode(Panel panel)
    {
        currentPanel = panel;
        code = new int[] { 0, 0, 0 };
    ...
    public (bool correct, bool completed) CheckCode(int digitGuess)
    {
        ...
        if (isCorrectGuess)
        {
            ...
            {
                var lightMaterial = currentPanel.lights[i].GetComponent<Renderer>().material;
    ...
    private void OpeningDoor(float t)
    {
    ...
    private void ClosingDoor(float t)
    {
        if (!door || !door2) return;
